...
class RepositoryController @Inject()(addToken: CSRFAddToken, authenticatedAction: AuthAction, repositoryService: RepositoryService)(implicit ec: ExecutionContext) extends InternationalInjectedController {

  private val defaultRedirect = Redirect(Pages.defaultLandingPage)

  def dashboard(folder: Option[Long], pageStart: Option[Int], pageSize: Option[Int], allRecords: Option[Boolean]) = addToken {
